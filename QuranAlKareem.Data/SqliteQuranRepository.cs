...
public sealed class SqliteQuranRepository : IQuranRepository
{
    ...
    public IReadOnlyList<string> FindRoots(string word)
    {
        ...
        // مطابقة تامّة فقط لتفادي النتائج الكاذبة. مطابقة الاحتواء (LIKE) خاطئة
        // لبحث الجذر لأنها تخلط بين السلسلة الفرعية والجذر:
        //   • «بخل» كانت تطابق «بِخَلْقِ» (الباء+خلق) فيظهر جذر «خلق».
        //   • «طه» (لا جذر لها) كانت تطابق «بَسَطَهُ» (بسط+هاء) فيظهر جذر «بسط».
        // لذا: 1) المُدخل جذر تامّ، وإلا 2) شكل/ليمة كلمة تامّ. ولا شيء غير ذلك.
